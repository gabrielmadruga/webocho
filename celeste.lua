      ...
      this.spd.y=appr(this.spd.y,this.dash_target.y,this.dash_accel.y)  
  	  ...
  	  if abs(this.spd.y) <= 0.15 then
          ...
          this.grace=0
					...
					-- wall jump
					local wall_dir=
            (this.is_solid(-3,0) and -1 or this.is_solid(3,0) and 1 or 0)
					if wall_dir!=0 then
            psfx(2)
            this.jbuffer=0
            this.spd.y=-2
...
set_hair_color=function(djump)
	pal(
    8,
    (djump==1 and 8 or djump==2 and (7+flr((frames/3)%2)*4) or 12)
  )
end
...
fall_floor = {
	...
	update=function(this)
		...
		if this.state == 0 then
			if 
        this.check(player,0,-1) or 
        this.check(player,-1,0) or 
