...
const sfxCount = 63;
let workFlowState: "normal" | "paused" | "looping" = "normal";
window.recordings = [];
let recordingPlayCursor = 0;
function addRecording() {
  // Don't want extra recordings after the cursor, as they would be parallel universes.
  if (recordingPlayCursor > 0) {
    window.recordings.length = recordingPlayCursor + 1;
  }
  window.recordings.push({
    engine: structuredClone(engineState),
    game: structuredClone(gameState),
  });
  recordingPlayCursor = window.recordings.length - 1;
}
function addRecordingForKeyCode(code: string) {
  // the assets check is neeaded to ensure the game state is initialized
  if (assets && workFlowState === "normal" && buttonMap.includes(code)) {
    addRecording();
  }
}
function offsetRecordingPlayCursor(offset: number) {
  const recordings = window.recordings;
  recordingPlayCursor = clamp(
    recordingPlayCursor + offset,
    0,
    recordings.length - 1
  );
  console.log(`Loading states for recordingPlayCursor: ${recordingPlayCursor}`);
  window.engineState = structuredClone(recordings[recordingPlayCursor].engine);
  window.gameState = structuredClone(recordings[recordingPlayCursor].game);
}
...
async function start() {
  ...
  loadAssets(game.name, function onLoadAssetsProgress(progressEvent) {
...
function loop(currentTime: DOMHighResTimeStamp) {
  if (!assets) {
    ...
    if (workFlowState === "looping") {
      if (recordingPlayCursor === window.recordings.length - 1) {
        offsetRecordingPlayCursor(-1);
      }
    }
    if (workFlowState === "normal" || workFlowState === "looping") {
      game.update();
      engineState.updateCount += 1;
      if (workFlowState === "looping") {
        if (
          engineState.updateCount ===
          window.recordings[recordingPlayCursor + 1].engine.updateCount
        ) {
          recordingPlayCursor += 1;
        }
      }
    }
    game.draw();
  ...
  // busy waiting sucks but setTimeout precision is horrible
  while (elapsedTime - msPerFrame < -0.1) {
...
async function loadAssets(
  ...
  const loadAudioPromise = loadAudio(game.name, sfxCount, audioPC);
  ...
  assets.sfxs = await Promise.all(
    assets.undecodedSfxs.map((s) => audioCtx!.decodeAudioData(s))
  );
  audioDecPC({
    loaded: 1,
    total: 1,
...
window.addEventListener("keydown", async function keydownListener(e) {
  if (workFlowState === "paused") {
    switch (e.code) {
      ...
      case "KeyL": {
        workFlowState = "looping";
        loop(performance.now());
        break;
      }
      case "KeyR": {
        console.log(window.recordings);
        console.log(recordingPlayCursor);
        break;
      }
    }
  } else if (workFlowState === "looping") {
    switch (e.code) {
      case "KeyP": {
        offsetRecordingPlayCursor(0);
        workFlowState = "paused";
        break;
      }
    }
  } else if (workFlowState === "normal") {
    engineState.buttons[e.code] = true;
    addRecordingForKeyCode(e.code);
    switch (e.code) {
      case "KeyP": {
        workFlowState = "paused";
        addRecording();
...
async function loadAudio(
  ...
  const promises = [];
  const progresEvents = new Map<string, { total: number; loaded: number }>();
    ...
    const promise = new Promise<ArrayBuffer>((resolve, reject) => {
      ...
      xhr.onprogress = progressCallback
        ? function onRequestProgress(event) {
            progresEvents.set(filename, event);
            if (progresEvents.size === sfxCount) {
              const accumulatedEvent = { total: 0, loaded: 0 };
              for (const event of progresEvents.values()) {
